...
async function evaluate_program(program: Program, environment: Environment): Promise<RuntimeValue> {
    let last_evaluated: RuntimeValue = { type: "null", value: null } as NullValue

    for (const statement of program.body) {
        last_evaluated = await evaluate(statement, environment)
    }
    return last_evaluated
}

async function evaluate_binary_expression(binary_operation: BinaryExpression, environment: Environment): Promise<RuntimeValue> {
    const left = await evaluate(binary_operation.left, environment)
    const right = await evaluate(binary_operation.right, environment)
...
async function evaluate_class_initialisation(class_call: Expression, environment: Environment): Promise<ClassInstanceValue> {
    ...
    const created_from_class = await evaluate((class_call as CallExpression).caller, environment)
    ...
    const argument_list = await Promise.all((class_call as CallExpression).arguments.map(async (arg) => await evaluate(arg, environment)))
...
async function evaluate_unary_expression(unary_operation: UnaryExpression, environment: Environment): Promise<RuntimeValue> {
    if (unary_operation.operator == "new") {
        return evaluate_class_initialisation(unary_operation.right, environment)
    }
    const right = await evaluate(unary_operation.right, environment)
...
async function call_procedure(procedure: ProcedureValue, args: RuntimeValue[]) {
    ...
    for (const statement of procedure.body) {
        await evaluate(statement, function_scope)
    }
}
...
async function evaluate_call_expression(call_expression: CallExpression, environment: Environment): Promise<RuntimeValue> {
    ...
    if (caller.type === "native-function") {
        ...
        const result = await func.call(argument_list, environment);
        return result;
        ...
        const object = await evaluate((call_expression.caller as MemberExpression).object, environment) as RuntimeValue
...
export async function evaluate(ast_node: Statement, environment: Environment): Promise<RuntimeValue> {
    switch (ast_node.kind) {
        ...
        case "CallExpression":
            return await evaluate_call_expression(ast_node as CallExpression, environment)
        ...
        case "ReturnStatement":
            environment.returnValue(await evaluate((ast_node as ReturnStatement).value, environment))
            return {
                type: "null",
                value: null
            } as NullValue
...
async function evaluate_for_statement(loop: ForLoop, environment: Environment): Promise<NullValue> {
    const loop_scope = new Environment(environment)
    const initial_value = await evaluate(loop.initial_value, environment)
    const end_value = await evaluate(loop.end_value, environment)
    ...
    while (true) {
        ...
        for (const statement of loop.body) {
            ...
            await evaluate(statement, loop_scope)
        }
...
async function evaluate_do_until_statement(loop: DoUntilLoop, environment: Environment): Promise<NullValue> {
    const loop_scope = new Environment(environment)

    while (true) {
        const condition_result = await evaluate(loop.condition, environment) as BooleanValue
...
async function evaluate_while_statement(loop: WhileLoop, environment: Environment): Promise<NullValue> {
    const loop_scope = new Environment(environment)
...
async function evaluate_variable_declaration(declaration: VariableDeclaration, environment: Environment) {
    const value = await evaluate(declaration.value as Expression, environment)
...
async function evaluate_array_declaration(declaration: ArrayDeclaration, environment: Environment) {
    const dimension_list: number[] = []
    for (const dimension of declaration.dimensions) {
        const value = await evaluate(dimension, environment)
...
async function evaluate_variable_assignment(assignment: AssignmentExpression, environment: Environment): Promise<RuntimeValue> {
    if (assignment.assign_to.kind == "Identifier") {
        const name = (assignment.assign_to as Identifier).symbol
        const value = await evaluate(assignment.value as Expression, environment)

        return environment.assignVariable(
            name, value
        )
    } else if (assignment.assign_to.kind == "MemberExpression") {
        const value = await evaluate(assignment.assign_to as MemberExpression, environment)
        const new_value = await evaluate(assignment.value as Expression, environment)
        ...
        updateObjKeepingRef(await evaluate(assignment.assign_to as MemberExpression, environment), new_value)
...
async function evaluate_member_expression(expression: MemberExpression, environment: Environment) {
    let object = await evaluate(expression.object, environment)
    ...
    if (type_prototype && type_prototype[string_property]) {
        ...
        const computed_property = await evaluate(expression.property, environment)
...
async function evaluate_range_expression(expression: RangeExpression, environment: Environment): Promise<RangeValue> {
...
async function evaluate_if_statement(if_statement: IfStatement, environment: Environment): Promise<RuntimeValue> {
    const condition_result = await evaluate(if_statement.condition, environment)
    if (is_truthy(condition_result)) {
        ...
        for (const statement of if_statement.then) {
            ...
            await evaluate(statement, block_environment)
        }
...
async function evaluate_switch_statement(switch_statement: SwitchStatement, environment: Environment): Promise<RuntimeValue> {
    const discriminant = await evaluate(switch_statement.discriminant, environment)

    const block_environment = new Environment(environment)
    for (const switch_case of switch_statement.cases) {
        const test = switch_case.test == null ? null : await evaluate(switch_case.test, environment)
